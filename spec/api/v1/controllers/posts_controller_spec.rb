...
RSpec.describe Api::V1::PostsController, type: :controller do
  ...
  context "unauthenticated user" do
    ...
    it "PUT update returns http unauthenticated" do
      put :update, id: my_post.id, post: { title: "Post title", body: "Post Body", topic_id: my_topic.id }
      expect(response).to have_http_status(401)
    end

    it "POST create returns http unauthenticated" do
      post :create, post: { title: "Post title", body: "Post Body", topic_id: my_topic.id }
      expect(response).to have_http_status(401)
    end
  ...
  context "unauthorized user" do
    ...
    it "POST create returns http forbiddin" do
      post :create, post: { title: "Post title", description: "Post Body" }
      expect(response).to have_http_status(403)
    end
